...
from automo.runtime import GraphRuntime, ModelGraphSpec, ModelNodeSpec, ModelOutputInput
...
def apply_graph_intervention(graph: ModelGraphSpec, intervention: ResearchIntervention) -> ModelGraphSpec:
    ...
    if intervention.kind == InterventionKind.NODE_MODEL:
        ...
        nodes = tuple(replace(node, model_spec_id=model_spec_id) if node.id == node_id else node for node in graph.nodes)
    ...
    if intervention.kind in {InterventionKind.MODEL_INPUT_ADDITION, InterventionKind.MODEL_INPUT_REMOVAL}:
        node_id = str(values["node_id"])
        ...
        for node in graph.nodes:
            ...
            if intervention.kind == InterventionKind.MODEL_INPUT_ADDITION:
                if not any(isinstance(item, ModelOutputInput) and item.node_id == upstream for item in inputs):
                    inputs.append(ModelOutputInput(upstream, alias=alias)); changed = True
            else:
                before = len(inputs)
                inputs = [item for item in inputs if not (isinstance(item, ModelOutputInput) and item.node_id == upstream)]
...
def evaluate_graph_candidate(
    ...
    *,
    ...
    baseline_val = next(iter(graph_runtime.evaluate(baseline.id, baseline_fit, rows=validation_rows, partition_id="validation").values()))
    candidate_val = next(iter(graph_runtime.evaluate(candidate.id, candidate_fit, rows=validation_rows, partition_id="validation").values()))
    ...
    validation_improvement = candidate_val - baseline_val if direction == MetricDirection.MAXIMIZE else baseline_val - candidate_val
    ...
    baseline_oos = next(iter(graph_runtime.evaluate(baseline.id, baseline_fit, rows=oos_rows, partition_id="research_oos").values()))
    candidate_oos = next(iter(graph_runtime.evaluate(candidate.id, candidate_fit, rows=oos_rows, partition_id="research_oos").values()))
    oos_improvement = candidate_oos - baseline_oos if direction == MetricDirection.MAXIMIZE else baseline_oos - candidate_oos
    return GraphCandidateEvaluation(intervention, candidate_val, candidate_oos, oos_improvement + 1e-15 >= minimum_improvement, candidate)
