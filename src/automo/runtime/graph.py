...
class LegacyTrainerAdapter:
    ...
    def fit(self, request: TrainingRequest) -> TrainingResult:
        ...
        return TrainingResult(
            predictor=LegacyPredictorAdapter(fitted), artifacts={"registry_model": fitted}
        )
...
class GraphRuntime:
    ...
    def fit(
        self, graph_id: str, *, rows: Sequence[Mapping[str, Any]], seed: int | None = None
    ) -> GraphTrainingResult:
        ...
        fold_assignments = self._fold_assignments(
            rows, graph.cross_fit.folds, graph.cross_fit.key, graph.cross_fit.seed
        )
    ...
    def evaluate(
        ...
        *,
        ...
        spec = self.runtime.model_specs[node.model_spec_id]
        resolved = self._prediction_inputs(
            node.inputs or spec.resolved_inputs(), rows, outputs, exclude=node.id
        )
    ...
    def register_graph(
        ...
        *,
        ...
        for node in self._topological_order(graph):
            ...
            if codec is None:
                raise GraphContractError(
                    f"trainer {spec.implementation} lacks artifact_codec required for graph registration"
                )
    ...
    def _cross_fit_output(
        ...
        *,
        ...
        if any(
            isinstance(inp, ModelOutputInput) for inp in (node.inputs or spec.resolved_inputs())
        ):
        ...
        for fold in range(graph.cross_fit.folds):
            ...
            holdout_indices = tuple(
                idx for idx in range(len(rows)) if fold_assignments[idx] == fold
            )
    ...
    def _base_inputs(
        self, inputs: Sequence[Any], rows: Sequence[Mapping[str, Any]]
    ) -> dict[str, Any]:
        resolved: dict[str, Any] = {}
        for inp in inputs:
            if isinstance(inp, FeatureSetInput):
                resolved[inp.alias] = self.runtime.features.materialize(
                    rows, self.runtime.feature_sets[inp.feature_set_id]
                )
            elif isinstance(inp, DataInput):
                resolved[inp.alias] = tuple(rows)
            elif isinstance(inp, ModelOutputInput):
    ...
    def _fold_assignments(
        rows: Sequence[Mapping[str, Any]], folds: int, key: str | None, seed: int
    ) -> tuple[int, ...]:
        values: list[int] = []
    ...
    def _topological_order(self, graph: ModelGraphSpec):
        ...
        deps = {
            node.id: {
                inp.node_id
                for inp in (
                    node.inputs or self.runtime.model_specs[node.model_spec_id].resolved_inputs()
                )
                if isinstance(inp, ModelOutputInput)
            }
