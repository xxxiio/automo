...
from .contracts import DataSnapshot, FeatureSpec, MetricDirection, ModelSpec
...
class CsvDataSource:
    ...
    def snapshot(self) -> DataSnapshot:
        ...
        return DataSnapshot(id=self.id, rows=rows, as_of_field=self.as_of_field, content_hash=content_hash)
...
class SingleFeatureLinearRunner:
    ...
    def fit(self, spec: ModelSpec, rows: Sequence[Mapping[str, Any]], *, target: Sequence[float]) -> _LinearModel:
        ...
        if len(features) != 1:
            raise ValueError("single-feature linear runner requires exactly one materialized feature")
        feature = features[0]
        ...
        slope = 0.0 if denom == 0 else sum((a-xbar)*(b-ybar) for a,b in zip(x,y, strict=True)) / denom
...
class MeanSquaredError:
    ...
    def evaluate(self, truth: Sequence[float], prediction: Sequence[float]) -> float:
        ...
        return sum((float(a)-float(b))**2 for a,b in zip(truth,prediction, strict=True)) / len(truth)
