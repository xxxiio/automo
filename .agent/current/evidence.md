...
| AC-003 | pass | `.github/workflows/ci.yml` executes the complete pre-commit configuration once and tests 3.11/3.12/3.13 separately; `scripts/init_dev.py` performs PPW-style dev sync plus clone-local hook installation; agent docs keep `uv run` sandbox-only |
