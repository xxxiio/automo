...
| G-004 | Wheel and sdist clean installs | not-run | previous setuptools artifact evidence invalidated by TASK-0020 uv_build migration; rebuild pending in connected CI |
...
```bash
python -m pytest -q
python scripts/health_gate.py --keep-dist  # current uv_build build blocked by unavailable build dependency
python -m pre_commit --version  # not available in this offline execution environment
```

## Checks not run

- uv_build wheel/sdist build-install smoke for the current candidate, plus committed uv.lock.
