...
def test_evidence_directed_plan_is_bounded_and_explicit(tmp_path: Path) -> None:
    ...
    service = ResearchService(
        runtime, split_strategies=runtime.plugin.split_strategies, store=store
    )
    plan = service.plan(
        iteration_id="RESEARCH-0001",
        baseline_model_spec_id="baseline",
        data_source_id="all",
        split_strategy_id="predefined",
        diagnosis="underfitting",
        findings=("baseline misses linear signal",),
        search_space=ResearchSearchSpace(
            "space",
            model_spec_ids=("baseline", "good-model"),
            feature_set_ids=("good",),
    ...
    assert all(
        c.intervention.kind in {InterventionKind.MODEL, InterventionKind.FEATURE_SET}
...
def test_candidate_history_blocks_duplicate_research(tmp_path: Path) -> None:
    ...
    kwargs = dict(
        ...
        data_source_id="all",
        split_strategy_id="predefined",
        diagnosis="feature_gap",
        findings=("missing useful feature",),
...
def test_progressive_evaluation_caps_oos_and_registers_accepted_model(tmp_path: Path) -> None:
    ...
    store = FilesystemResearchStore(tmp_path / ".automo/research")
    service = ResearchService(
        runtime, split_strategies=runtime.plugin.split_strategies, store=store, registry=registry
    )
    plan = service.plan(
        iteration_id="RESEARCH-0003",
        baseline_model_spec_id="baseline",
        data_source_id="all",
        split_strategy_id="predefined",
        diagnosis="underfitting",
        findings=("baseline underfits",),
        search_space=ResearchSearchSpace(
            "space", model_spec_ids=("good-model",), feature_set_ids=("good",)
        ),
        budget=ResearchBudget(maximum_candidates=2, maximum_model_fits=4, maximum_oos_candidates=1),
        safeguards=ResearchSafeguards(
            minimum_improvement=0.01, minimum_validation_observations=2, minimum_oos_observations=2
        ),
    )
...
def test_calibration_search_is_allowed_for_calibration_diagnosis(tmp_path: Path) -> None:
    ...
    plan = service.plan(
        iteration_id="RESEARCH-0004",
        baseline_model_spec_id="good-model",
        data_source_id="all",
        split_strategy_id="predefined",
        diagnosis="poor_calibration",
        findings=("calibration drift",),
...
def test_research_cli_plan_run_history(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    executed = runner.invoke(
        app, ["research", "run", "R-CLI", "--space", "default", "--root", str(tmp_path)]
    )
...
def test_parameter_intervention_executes_within_budget(tmp_path: Path) -> None:
    ...
    plan = service.plan(
        iteration_id="RESEARCH-0005",
        baseline_model_spec_id="baseline",
        data_source_id="all",
        split_strategy_id="predefined",
        diagnosis="overfitting",
        findings=("regularize candidate",),
...
def test_missing_candidate_capability_persists_bounded_request(tmp_path: Path) -> None:
    ...
    plan = service.plan(
        iteration_id="RESEARCH-0006",
        baseline_model_spec_id="baseline",
        data_source_id="all",
        split_strategy_id="predefined",
        diagnosis="underfitting",
        findings=("try new family",),
