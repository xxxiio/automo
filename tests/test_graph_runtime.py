...
class ExternalFitService:
    ...
    def score_actions(self, rows, actions):
        ...
        return sum(1.0 for row, action in zip(rows, actions, strict=True) if action["take"] == (float(row["signal"]) >= 0.5)) / len(rows)
...
class ActionPredictor:
    ...
    def predict(self, request: PredictionRequest) -> ModelOutputBatch:
        return ModelOutputBatch(tuple({"take": float(row["signal"]) >= self.threshold} for row in request.rows), output_name="action")
...
class ActionAccuracyEvaluator:
    ...
    def evaluate(self, context: EvaluationContext) -> float:
        return float(context.services["external_fit"].score_actions(context.rows, context.outputs.values))
...
def test_custom_trainer_and_structured_evaluator_use_external_service() -> None:
    ...
    spec = ModelSpec("action-model", "external.action", None, objective, EvaluationSpec(metric), inputs=(DataInput(),))
    source = MemorySource("train", ({"signal": 0.1}, {"signal": 0.9}, {"signal": 0.8}, {"signal": 0.2}))
    plugin = ResearchPlugin(
        id="external-example", data_sources=(source,), feature_computers=(), feature_sets=(), objectives=(objective,),
        metrics=(), model_specs=(spec,), model_runners=(), model_trainers=(ExternalActionTrainer(),),
        evaluators=(ActionAccuracyEvaluator(),), services={"external_fit": service},
    )
...
class MemorizingPredictor:
    def __init__(self, memory): self.memory = dict(memory)
    def predict(self, request: PredictionRequest) -> ModelOutputBatch:
...
class MemorizingTrainer:
    ...
    def fit(self, request: TrainingRequest) -> TrainingResult:
        return TrainingResult(MemorizingPredictor((row["id"], float(row["y"])) for row in request.rows))


class MetaPredictor:
    def __init__(self, offset: float): self.offset = offset
    def predict(self, request: PredictionRequest) -> ModelOutputBatch:
...
class LeakageCheckingMetaTrainer:
    ...
    def __init__(self) -> None: self.seen_upstream: tuple[float, ...] | None = None
    def fit(self, request: TrainingRequest) -> TrainingResult:
...
def test_meta_model_training_uses_cross_fitted_upstream_outputs() -> None:
    ...
    base = ModelSpec("base", "memorize", None, base_obj, EvaluationSpec(metric), inputs=(DataInput(),))
    meta = ModelSpec("meta", "meta.check", None, meta_obj, EvaluationSpec(metric), inputs=(ModelOutputInput("base", alias="base"),))
    meta_trainer = LeakageCheckingMetaTrainer()
    graph = ModelGraphSpec(
        "stack", (ModelNodeSpec("base", "base"), ModelNodeSpec("meta", "meta")), "meta", CrossFitSpec(folds=3, key="id", seed=11)
    )
    plugin = ResearchPlugin(
        id="stacking-example", data_sources=(source,), feature_computers=(), feature_sets=(), objectives=(base_obj, meta_obj),
        metrics=(), model_specs=(base, meta), model_runners=(), model_trainers=(MemorizingTrainer(), meta_trainer),
        evaluators=(NumericEvaluator(),), model_graphs=(graph,),
    )
    runtime = ResearchRuntime(plugin)
...
def test_graph_interventions_can_change_node_model_and_upstream_inputs() -> None:
    ...
    metric = MetricSpec("mse2", MetricDirection.MINIMIZE)
    base = ModelSpec("base-a", "memorize", None, obj, EvaluationSpec(metric), inputs=(DataInput(),))
    base_b = ModelSpec("base-b", "memorize", None, obj, EvaluationSpec(metric), inputs=(DataInput(),))
    meta = ModelSpec("meta", "meta.check", None, obj, EvaluationSpec(metric), inputs=(ModelOutputInput("base", alias="base"),))
    graph = ModelGraphSpec("g", (ModelNodeSpec("base", "base-a"), ModelNodeSpec("meta", "meta", inputs=(ModelOutputInput("base", alias="base"),))), "meta")
    changed = apply_graph_intervention(graph, ResearchIntervention(InterventionKind.NODE_MODEL, {"node_id": "base", "model_spec_id": "base-b"}))
    assert changed.nodes[0].model_spec_id == "base-b"
    removed = apply_graph_intervention(graph, ResearchIntervention(InterventionKind.MODEL_INPUT_REMOVAL, {"node_id": "meta", "upstream_node_id": "base"}))
...
def test_graph_registration_records_upstream_model_identity_and_cross_fit() -> None:
    ...
    meta = ModelSpec("meta", "meta.check", None, obj, EvaluationSpec(metric), inputs=(ModelOutputInput("base", alias="base"),))
    graph = ModelGraphSpec("stack", (ModelNodeSpec("base", "base"), ModelNodeSpec("meta", "meta")), "meta", CrossFitSpec(folds=2, key="id", seed=2))
    plugin = ResearchPlugin(
        id="p", data_sources=(source,), feature_computers=(), feature_sets=(), objectives=(obj,), metrics=(),
        model_specs=(base, meta), model_runners=(), model_trainers=(RegisterableBase(), RegisterableMeta()),
        evaluators=(NumericEvaluator(),), model_graphs=(graph,),
    )
    runtime = ResearchRuntime(plugin)
    ...
    ids = gr.register_graph("stack", fitted, registry=registry, data_source_id="fit", data_snapshot_id="snap", data_snapshot_hash="hash")
