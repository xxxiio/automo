...
from automo.refresh import ModelPoolSpec, ModelSelector, RetentionPolicy, SelectionPolicy, TrainingPolicy, CalibrationPolicy
...
def test_data_only_model_provenance_can_omit_feature_set() -> None:
    provenance = TrainingProvenance(
        data_source_id="data", data_snapshot_id="snap", data_snapshot_hash=None, feature_set_id=None,
        model_spec_id="raw", objective_id="obj", runner_implementation="custom", python_version="3.13",
    )
    manifest = ModelManifest(
        id="MODEL-1", implementation="custom", model_spec_id="raw", objective_id="obj",
        feature_set_id=None, artifact_codec="codec", artifact_path="x", artifact_hash="h", provenance_path="p",
    )
...
def test_pool_can_describe_heterogeneous_comparable_specs() -> None:
    pool = ModelPoolSpec(
        id="pool", objective_id="obj", model_spec_id=None, model_spec_ids=("a", "b"),
        primary_metric_id="m", primary_metric_direction=MetricDirection.MAXIMIZE,
        training_policy=TrainingPolicy("train"), calibration_policy=CalibrationPolicy("cal"),
        retention_policy=RetentionPolicy("keep"), selection_policy=SelectionPolicy("select"),
    )
...
def _project_agent(root: Path) -> None:
    ...
    (base / "research/ranking.md").write_text("# Ranking research\nUse point-in-time evidence.\n", encoding="utf-8")
    (base / "index.json").write_text(json.dumps({
        "schema_version": 1, "infer": [],
        "rules": [{"id": "ranking", "concerns": ["model-research", "experiment-design"], "load": ["research/ranking.md"]}],
    }), encoding="utf-8")
...
def test_guidance_lock_detects_project_agent_drift(tmp_path: Path) -> None:
    ...
    (tmp_path / ".project-agent/automo/research/ranking.md").write_text("# Ranking research\nChanged.\n", encoding="utf-8")
...
def test_project_agent_rejects_path_escape(tmp_path: Path) -> None:
    ...
    (base / "index.json").write_text(json.dumps({"schema_version": 1, "rules": [{"id": "bad", "concerns": ["experiment-design"], "load": ["../secret.md"]}]}), encoding="utf-8")
...
def test_guidance_cli_matches_getdone_style_project_agent_switch(tmp_path: Path) -> None:
    _project_agent(tmp_path)
    result = RUNNER.invoke(app, ["guidance", "--task-class", "experiment-design", "--root", str(tmp_path), "--paths-only"])
    assert result.exit_code == 0
    assert ".project-agent/automo/research/ranking.md" in result.stdout
    result = RUNNER.invoke(app, ["guidance", "--task-class", "experiment-design", "--root", str(tmp_path), "--no-project-agent", "--paths-only"])
    assert result.exit_code == 0
...
def test_project_agent_infers_concerns_from_changed_paths(tmp_path: Path) -> None:
    ...
    docs = select_guidance("experiment-design", project_root=tmp_path, changed_paths=("src/data/feed.py",))
...
def test_research_plan_conformance_rejects_confounded_or_duplicate_work() -> None:
    from automo.research import (
        CandidateProposal, InterventionKind, ResearchBudget, ResearchIntervention, ResearchPlan,
        ResearchSafeguards, ResearchSearchSpace, validate_research_plan,
    )
    intervention = ResearchIntervention(InterventionKind.PARAMETERS, {"depth": 3})
    candidate = CandidateProposal("c1", "base", intervention, ("diagnosis",), "improve metric", ("no lift",), 1)
    plan = ResearchPlan(
        "p", "base", "data", "split", "diagnosis", (),
        ...
        (candidate, CandidateProposal("c2", "base", intervention, ("same",), "same", ("no lift",), 2)),
    )
...
def test_automo_ignores_root_project_agent_owned_by_getdone(tmp_path: Path) -> None:
    ...
    (root_agent / "index.json").write_text(json.dumps({
        "schema_version": 1,
        "rules": [{"id": "dev", "concerns": ["experiment-design"], "load": ["getdone-only.md"]}],
    }), encoding="utf-8")
    docs = select_guidance("experiment-design", project_root=tmp_path)
