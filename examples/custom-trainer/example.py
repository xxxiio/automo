from automo.runtime import (
    DataInput, DataSnapshot, EvaluationContext, EvaluationSpec, MetricDirection, MetricSpec,
    ModelOutputBatch, ModelSpec, ObjectiveSpec, PredictionRequest, ResearchPlugin, ResearchRuntime,
...
class Predictor:
    ...
    def predict(self, request: PredictionRequest):
        return ModelOutputBatch(tuple(float(row["x"]) >= self.threshold for row in request.rows), "decision")
...
class Evaluator:
    ...
    def evaluate(self, context: EvaluationContext):
        ...
        return sum(a == b for a, b in zip(expected, context.outputs.values, strict=True)) / len(expected)
...
spec = ModelSpec("model", "example.external", None, objective, EvaluationSpec(metric), inputs=(DataInput(),))
plugin = ResearchPlugin(
    id="custom-trainer", data_sources=(Source(),), feature_computers=(), feature_sets=(), objectives=(objective,),
    metrics=(), model_specs=(spec,), model_runners=(), model_trainers=(Trainer(),), evaluators=(Evaluator(),),
