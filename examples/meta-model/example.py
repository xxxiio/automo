...
class BaseTrainer:
    ...
    def fit(self, request: TrainingRequest):
        seen = ((row["id"], float(row["y"])) for row in request.rows)
        return TrainingResult(BasePredictor(seen))


class MetaPredictor:
    def predict(self, request: PredictionRequest):
        values = tuple(float(v) + 2.0 for v in request.inputs["base"].values)
        return ModelOutputBatch(values)
...
class MetaTrainer:
    ...
    def fit(self, request: TrainingRequest):
        # Upstream fit inputs are OOF predictions; memorized training values do not leak.
...
class MSE:
    ...
    def evaluate(self, context: EvaluationContext):
        pairs = zip(context.rows, context.outputs.values, strict=True)
        squared_errors = ((float(row["y"]) - float(pred)) ** 2 for row, pred in pairs)
        return sum(squared_errors) / len(context.rows)
...
plugin = ResearchPlugin(
    ...
    model_runners=(),
    model_trainers=(BaseTrainer(), MetaTrainer()),
    evaluators=(MSE(),),
    model_graphs=(graph,),
)
